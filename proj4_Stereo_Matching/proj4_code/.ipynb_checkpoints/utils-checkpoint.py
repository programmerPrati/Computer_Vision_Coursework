...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
    ...
    # Create a single-channel random 0/1 image, then repeat across channels so it's grayscale
    single = torch.randint(0, 2, (H, W), dtype=torch.float32)  # 0 or 1
    im_left = single.unsqueeze(-1).repeat(1, 1, C)  # shape (H, W, C)
    
    # Start im_right as a copy of im_left
    im_right = im_left.clone()
    
    # Compute central block top-left corner
    center_y = H // 2
    center_x = W // 2
    top = center_y - (block_h // 2)
    left = center_x - (block_w // 2)
    
    # Destination left position after shifting block to the left by `disparity`
    dest_left = left - disparity
    dest_top = top  # vertical position doesn't change
    
    # Place the shifted block into the right image
    # Source block coordinates (in im_left)
    src_y0, src_y1 = top, top + block_h
    src_x0, src_x1 = left, left + block_w
    
    # Destination block coordinates (in im_right)
    dst_y0, dst_y1 = dest_top, dest_top + block_h
    dst_x0, dst_x1 = dest_left, dest_left + block_w
    
    # Copy the block (guaranteed by prompt that move stays in bounds)
    im_right[dst_y0:dst_y1, dst_x0:dst_x1, :] = im_left[src_y0:src_y1, src_x0:src_x1, :]
    
    # Fill the hole (the original block location) with random 0/1 grayscale values
    hole = torch.randint(0, 2, (block_h, block_w), dtype=torch.float32)
    im_right[src_y0:src_y1, src_x0:src_x1, :] = hole.unsqueeze(-1).repeat(1, 1, C)
...
def vis_image_scales_numpy(image: np.ndarray) -> np.ndarray:
  ...
  for scale in range(2, scales + 1):
    ...
    cur_image =  np.array(Image.fromarray((cur_image * 255).astype(np.uint8)).resize((new_w, new_h))) / 255.0
